...
fused_norm_and_clip_kernel(
    ...
    for (int tensor_idx = 0; tensor_idx < num_tensors; tensor_idx++) {
        ...
        const int size = grad_sizes[tensor_idx];
        
        // Vectorized processing
        const int vec_size = size / 4;
        const int vec_start = (blockIdx.x * blockDim.x + threadIdx.x);
        const int vec_stride = gridDim.x * blockDim.x;
        
        // Process float4 chunks
        for (int i = vec_start; i < vec_size; i += vec_stride) {
            float4 vals = __ldg(reinterpret_cast<const float4*>(&grad[i * 4]));
            thread_norm_sq += vals.x * vals.x + vals.y * vals.y + 
                             vals.z * vals.z + vals.w * vals.w;
        }
        
        // Handle remainder
        const int remainder_start = vec_size * 4;
        for (int i = remainder_start + threadIdx.x + blockIdx.x * blockDim.x; 
             i < size; 
    ...
    if (lane == 0) smem[wid] = thread_norm_sq;
    __syncthreads();
...
float fused_grad_clip_launcher(
    ...
    fused_norm_and_clip_kernel<<<blocks, threads, 0, stream>>>(
        grad_ptrs_device, grad_sizes_device, num_tensors,
        max_norm, norm_buffer
    );
