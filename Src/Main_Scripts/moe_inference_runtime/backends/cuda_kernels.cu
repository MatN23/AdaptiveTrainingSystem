...
// Initialize CUDA
__attribute__((constructor)) static void init_cuda() {
  int device_count = 0;
...
// Cleanup
__attribute__((destructor)) static void cleanup_cuda() {
  if (g_cublas_handle) {
...
// Scaled add kernel
__global__ void add_scaled_kernel(float *dst, const float *src, float scale,
                                  int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
// RoPE kernel
__global__ void rope_kernel(float *q, float *k, const float *cos_cache,
                            const float *sin_cache, int seq_len, int num_heads,
                            int num_kv_heads, int head_dim, int pos_offset) {
  int h = blockIdx.x;
  int s = blockIdx.y;
  int d = threadIdx.x;
...
__global__ void attention_softmax_kernel(float *scores, int seq_len,
                                         int kv_seq_len, int cache_pos) {
  int s = blockIdx.x;
  int tid = threadIdx.x;
  if (s >= seq_len)
    return;
  ...
  if (valid_len > kv_seq_len)
    valid_len = kv_seq_len;
...
__global__ void moe_softmax_topk_kernel(const float *logits,
                                        int32_t *expert_ids,
                                        float *expert_weights, int seq_len,
                                        int num_experts, int top_k) {
  constexpr int MAX_TOP_K = 32;
  int token_idx = blockIdx.x;
  if (token_idx >= seq_len || threadIdx.x != 0)
    return;
...
__global__ void moe_gating_kernel(const float *x, const float *gate_weight,
                                  float *logits, int seq_len, int hidden_size,
                                  int num_experts) {
  int t = blockIdx.x;
  int e = blockIdx.y;

  if (t >= seq_len || e >= num_experts)
    return;
...
__global__ void mod_scoring_kernel(const float *x, const float *router_weight,
                                   float *scores, int seq_len,
                                   int hidden_size) {
  int t = blockIdx.x;
  if (t >= seq_len)
    return;
...
void rms_norm(const float *x, const float *weight, float *out, int32_t n,
...
void matmul(const float *a, const float *b, float *c, int32_t m, int32_t k,
            int32_t n) {
...
void apply_rope(float *q, float *k, const float *cos_cache,
                const float *sin_cache, int32_t seq_len, int32_t num_heads,
                int32_t num_kv_heads, int32_t head_dim, int32_t pos_offset) {
  dim3 blocks(std::max(num_heads, num_kv_heads), seq_len);
  dim3 threads(head_dim / 2);

  rope_kernel<<<blocks, threads>>>(q, k, cos_cache, sin_cache, seq_len,
                                   num_heads, num_kv_heads, head_dim,
                                   pos_offset);
}

void attention(const float *q, const float *k, const float *v, float *out,
               float *kv_cache_k, float *kv_cache_v, int32_t seq_len,
  ...
  for (int h = 0; h < num_kv_heads; ++h) {
    cudaMemcpyAsync(kv_cache_k + (h * kv_seq_len + cache_pos) * head_dim,
                    k + h * seq_len * head_dim,
                    seq_len * head_dim * sizeof(float),
                    cudaMemcpyDeviceToDevice, 0);
    cudaMemcpyAsync(kv_cache_v + (h * kv_seq_len + cache_pos) * head_dim,
  ...
  for (int h = 0; h < num_heads; ++h) {
    ...
    cublasSgemm(g_cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, kv_seq_len, seq_len,
                head_dim, &alpha, kv_cache_k + kv_h * kv_seq_len * head_dim,
                head_dim, q + h * seq_len * head_dim, head_dim, &beta, scores,
                kv_seq_len);

    // Softmax
    attention_softmax_kernel<<<seq_len, 256>>>(scores, seq_len, kv_seq_len,
                                               cache_pos);
    ...
    cublasSgemm(g_cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, head_dim, seq_len,
                kv_seq_len, &one, kv_cache_v + kv_h * kv_seq_len * head_dim,
                head_dim, scores, kv_seq_len, &zero,
...
void moe_route(const float *x, const float *gate_weight, int32_t *expert_ids,
               float *expert_weights, int32_t seq_len, int32_t hidden_size,
  ...
  dim3 threads(256);
  moe_gating_kernel<<<blocks, threads>>>(x, gate_weight, d_logits, seq_len,
                                         hidden_size, num_experts);

  moe_softmax_topk_kernel<<<seq_len, 1>>>(
      d_logits, d_expert_ids, d_expert_weights, seq_len, num_experts, top_k);

  cudaMemcpy(expert_ids, d_expert_ids, seq_len * top_k * sizeof(int32_t),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(expert_weights, d_expert_weights, seq_len * top_k * sizeof(float),
             cudaMemcpyDeviceToHost);
...
void mod_route(const float *x, const float *router_weight, float *scores,
               int32_t seq_len, int32_t hidden_size) {
  float *d_scores = nullptr;
  cudaMalloc(&d_scores, seq_len * sizeof(float));
  mod_scoring_kernel<<<seq_len, 256>>>(x, router_weight, d_scores, seq_len,
                                       hidden_size);
  cudaMemcpy(scores, d_scores, seq_len * sizeof(float), cudaMemcpyDeviceToHost);
  cudaFree(d_scores);
}

void expert_forward(const float *x, float *out, const float *gate_w,
                    const float *up_w, const float *down_w, int32_t hidden_size,
  ...
  cublasSgemv(g_cublas_handle, CUBLAS_OP_T, hidden_size, intermediate_size,
              &one, gate_w, hidden_size, x, 1, &zero, gate_out, 1);

  // Up projection: up_out = x @ up_w^T
  cublasSgemv(g_cublas_handle, CUBLAS_OP_T, hidden_size, intermediate_size,
              &one, up_w, hidden_size, x, 1, &zero, up_out, 1);

  // SwiGLU: gate_out = silu(gate_out) * up_out
  silu_kernel<<<(intermediate_size + 255) / 256, 256>>>(gate_out, gate_out,
                                                        intermediate_size);
  mul_kernel<<<(intermediate_size + 255) / 256, 256>>>(
      gate_out, up_out, gate_out, intermediate_size);

  // Down projection: out = gate_out @ down_w^T
  cublasSgemv(g_cublas_handle, CUBLAS_OP_T, intermediate_size, hidden_size,
              &one, down_w, intermediate_size, gate_out, 1, &zero, out, 1);
}
