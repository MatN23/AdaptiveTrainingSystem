...
// Initialize Metal
__attribute__((constructor)) static void init_metal() {
  @autoreleasepool {
...
static id<MTLBuffer> create_buffer(const float *data, size_t count) {
  ...
  if (data) {
    return [g_device newBufferWithBytes:data
                                 length:bytes
...
void rms_norm(const float *x, const float *weight, float *out, int32_t n,
  ...
  @autoreleasepool {
    // RMSNorm - using CPU implementation (MPS doesn't have native RMSNorm)
    // TODO: Write custom Metal compute shader for better performance
...
void matmul(const float *a, const float *b, float *c, int32_t m, int32_t k,
  ...
  @autoreleasepool {
    ...
    printf("debug: matmul m=%d k=%d n=%d\n", m, k, n);
    ...
    // b is [n, k] (out_features, in_features)
    ...
    MPSMatrix *mat_a = [[MPSMatrix alloc] initWithBuffer:a_buf
                                              descriptor:desc_a];
    MPSMatrix *mat_b = [[MPSMatrix alloc] initWithBuffer:b_buf
                                              descriptor:desc_b];
    MPSMatrix *mat_c = [[MPSMatrix alloc] initWithBuffer:c_buf
                                              descriptor:desc_c];
